...
def is_unique(packet):
    ...
    if len(packet) == 4:
        for j in range(0, len(packet)):
            if len(packet.replace(packet[j], "")) < 3:
                unique = False
...
with open('input.txt') as f:
    ...
    for line in lines:
        ...
        for i in range(4, len(line_clean)):
            check_string = line_clean[i - 4:i:]
            if is_unique(check_string):
                print("found start-of-packet {} at character {}".format(check_string, i))
                break
