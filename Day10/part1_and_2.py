...
class CPU:
    def __init__(self):
        self.cycle = 0
        self.x = 1
        self.signal_hist = [0 for i in range(100000)]
        self.screen_width = 40

    def tick(self):
        self.cycle += 1
        self.signal_hist[self.cycle] = self.signal()
        self.show()
    ...
    def show(self):
        column = (self.cycle - 1) % self.screen_width

        # check if it's visible
        if self.x - 1 <= column <= self.x + 1:
            print("⬜", end="")
        else:
            print("⬛", end="")

        # print a new line if we're at the edge
        if column == self.screen_width - 1:
            print()

    def get_signal_sum(self, cycles_to_inspect):
        total = 0
        for i in range(self.cycle):
            if i in cycles_to_inspect:
                total += self.signal_hist[i]
...
print(computer.get_signal_sum([20, 60, 100, 140, 180, 220]))