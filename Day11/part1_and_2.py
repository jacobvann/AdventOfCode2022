...
# need this for floor and for lcm
import math


class Monkey:
    def __init__(self, index, items, operator, operand, divisibility, throw_on_true, throw_on_false, divide_by_three=False, debug_print=False):
        self.index = index
        self.items = items
    ...
    def print(self):
        print("Monkey {}:".self.index)
        print("  Starting items: {}".format(", ".join(self.items)))
        print("  Operation: new = old {} {}".format(self.operator, self.operand))
        print("  Test: divisible by {}".format(self.divisibility))
        print("    If true: throw to monkey {}".format(self.throw_on_true))
        print("    If false: throw to monkey {}".format(self.throw_on_false))
        print()

    def inspect(self):
        # go through items one at a time
        if self.debug_print:
            print("Monkey {} Inspecting!".format(self.index))

        for item in self.items:

            # initialize worry value
            worry_value = int(item)
            if self.debug_print:
                print("--> ITEM {} (worry value: {})".format(item, worry_value))

            # 1. do operation
            worry_operand = worry_value if self.operand == 'old' else int(self.operand)
            if self.operator == '*':
                worry_value = (worry_value * int(worry_operand)) % LCM
                if self.debug_print:
                    print(" .. multiplying by {} to get {}".format(int(worry_operand), worry_value))
            elif self.operator == '+':
                worry_value = (worry_value + int(worry_operand)) % LCM
                if self.debug_print:
                    print(" .. adding {} to get {}".format(int(worry_operand), worry_value))

            # # 2. divide by 3 (round down)
            if self.divide_by_three:
                worry_value = math.floor(int(worry_value) / 3)
                if self.debug_print:
                    print(" .. dividing by 3 to get {}".format(worry_value))

            # 3. test & throw
            if worry_value % int(self.divisibility) == 0:
                Monkeys[self.throw_on_true].items.append(worry_value)
                if self.debug_print:
                    print(" .. YES it is divisible by {}".format(self.divisibility))
                    print(" .. Throwing {} to monkey {}\n".format(worry_value, self.throw_on_true))
            else:
                Monkeys[self.throw_on_false].items.append(worry_value)
                if self.debug_print:
                    print(" .. NO it IS NOT divisible by {}".format(self.divisibility))
                    print(" .. Throwing {} to monkey {}\n".format(worry_value, self.throw_on_false))

            self.items_inspected += 1

        # clear the list since all were thrown
        self.items = []


# no shame in hard-coding the input
# if I had more time it would have been fun to make a parser
# create some monkeys
Monkeys = [Monkey(0, ['80'], '*', 5, 2, 4, 3),
           Monkey(1, ['75', '83', '74'], '+', 7, 7, 5, 6),
           Monkey(2, ['86', '67', '61', '96', '52', '63', '73'], '+', 5, 3, 7, 0),
           Monkey(3, ['85', '83', '55', '85', '57', '70', '85', '52'], '+', 8, 17, 1, 5),
           Monkey(4, ['67', '75', '91', '72', '89'], '+', 4, 11, 3, 1),
           Monkey(5, ['66', '64', '68', '92', '68', '77'], '*', 2, 19, 6, 2),
           Monkey(6, ['97', '94', '79', '88'], '*', 'old', 5, 2, 7),
           Monkey(7, ['77', '85'], '+', 6, 13, 4, 0)]

LCM_input = []

# calculate Least common multiple of all the divisors
# we will use this to keep the worry level from going astronomical
# Divisibility to remain the same if you mod by the LCM
for m in range(len(Monkeys)):
    LCM_input.append(Monkeys[m].divisibility)

LCM = math.lcm(*LCM_input)

print("========== STARTING =============")
for m in range(len(Monkeys)):
    print("Monkey {}: {}".format(m, Monkeys[m].items))
...
# to keep track of 1st and 2nd place
busiest_monkeys = [0] * len(Monkeys)
for m in range(len(Monkeys)):
    print("Monkey {} inspected {} items".format(m, Monkeys[m].items_inspected))
    busiest_monkeys[m] = int(Monkeys[m].items_inspected)

busiest_monkeys.sort(reverse=True)
total_monkey_business = busiest_monkeys[0] * busiest_monkeys[1]

print ("Total Monkey business from top two monkeys = {}".format(total_monkey_business))